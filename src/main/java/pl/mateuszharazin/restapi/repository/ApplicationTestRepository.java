...
//    At the beginning test solving
    ...
    @Modifying
    @Query(value = "UPDATE test_result SET (test_score = :score, solution_date = CURRENT_DATE) WHERE application_id = :appId;", nativeQuery = true)
    @Transactional
    public void updateAfterTest(@Param("appId") int applicationId, @Param("score") int testScore);
